/**
 * COSC 3250 - Project 4
 * program to test functionality of create.c and ctxsw.S
...
void testcases(void)
{
	...
	for(int i=top; i >= 0; i--)
	{
		ulong * ptr = (ulong*)ppcb->stkbase + i;
		kprintf("address: 0x%08X contents %08X\n", ptr, ptr*);
	}
	/*pct * pbegin = ppcb->stkbase;
	pct * pend = ppcb->stkbase - stklen;
	for(ulong * itptr = begin; itptr < end; itptr--)
	{
		itptr->state;
		itptr-name;
	}
