...
void xtrap(long *frame, int cause)
{
	...
	/* pseudocode:
 * 		else if (ARM_EXCEPTION_SWI)
 * 		{
 * 			//FIND SWI ADDRESS IN FRAME:
 * 			SWI_ADDRESS=FRAME[N];
 * 			FRAME[N] += 4
 *			//RESET RETURN REGISTER IN FRAME
 *			FRAME[RET_REG]=SYSCALL_DISPATCH(?,?);
 *			RETURN;
 * 		}
 * 		*/
	else if(cause == ARM_EXCEPTION_SWI)
	{
		...
		frame[CTX_R0] = syscall_dispatch( 0xffffff & var, frame);
		frame[CTX_LR] = frame[CTX_LR] + 4;
